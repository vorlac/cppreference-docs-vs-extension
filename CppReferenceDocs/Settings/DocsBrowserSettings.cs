...
{
    [Export(typeof(IDocsBrowserSettings))]
    public sealed class DocsBrowserSettings : IDocsBrowserSettings
    {
        private const string DefaultHomePage = "https://www.cppreference.com/";
        private const string SettingsKey = nameof(DocsBrowserSettings);

        private readonly ILogger _log = Log.Logger;
        private readonly WritableSettingsStore _settingsStore;

        [ImportingConstructor]
        public DocsBrowserSettings(SVsServiceProvider vsServiceProvider)
        {
            try
            {
                ...
                if (_settingsStore == null)
                    _log.Error(
                        $"{nameof(DocsBrowserSettings)} Constructor: could not retrieve an instance of {nameof(WritableSettingsStore)}");
            }
            catch (Exception ex)
            {
                _log.Error(ex,
        ...
        public void Load()
        {
            try
            {
                ...
                MinimumLogLevel = (LogEventLevel)_settingsStore.GetInt32(SettingsKey, nameof(MinimumLogLevel),
                    (int)LogEventLevel.Verbose);
            }
            catch (Exception ex)
            {
                _log.Error(ex, $"{nameof(DocsBrowserSettings)}: Failed to load settings");
            }
        }
        ...
        public void Save()
        {
            ...
            {
                _log.Error(ex, $"{nameof(DocsBrowserSettings)}: Failed to save settings");
            }
        }
