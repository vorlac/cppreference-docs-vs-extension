...
internal static class DocsDownloader {
    ...
    private static readonly Uri CppReferenceDocReleasesUri =
        new(@"https://api.github.com/repos/PeterFeicht/cppreference-doc/tags");
    ...
    private static bool CreateDirectory(Uri dirPath) {
        var tempDownloadDir = dirPath.ToString();
    ...
    public static void FetchCppRefDocs(Uri uri) {
        ...
        using var fs = new FileStream(uri.AbsolutePath + "cppref-docs-releases.txt", FileMode.OpenOrCreate);
        s.Result.CopyTo(fs);
    }
}
